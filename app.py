        ...
        if st.button("🚀 予測を開始する"):
            ...
            if not df_res.empty:
                ...
                for _, row in df_res.iterrows():
                    radius = 5 + (row['予測人数'] * 1.5)
                    color = 'red' if row['予測人数'] >= 20 else 'orange' if row['予測人数'] >= 5 else 'blue'
                    folium.CircleMarker(
                ...
                st.dataframe(
                    df_res.sort_values("予測人数", ascending=False).drop(['lat', 'lon'], axis=1).style.applymap(color_risk, subset=['予測人数']),
